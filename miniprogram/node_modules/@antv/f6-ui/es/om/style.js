...
function () {
      ...
      for (var _b = 0, _c = rule.selectors; _b < _c.length; _b++) {
        ...
        switch (keySeletor[0]) {
          case '#':
            ...
            rulesHash.ids[key] = __spreadArray(__spreadArray([], rulesHash.ids[key] || []), [{
              selector: selector,
          ...
          case '.':
            ...
            rulesHash.classes[classKey] = __spreadArray(__spreadArray([], rulesHash.classes[classKey] || []), [{
              selector: selector,
              specificity: this.genSpecificity(selector),
              style: this.genStyleFromRule(rule)
            }]);
          ...
          default:
            rulesHash.tagNames[keySeletor] = __spreadArray(__spreadArray([], rulesHash.tagNames[keySeletor] || []), [{
              selector: selector,
              specificity: this.genSpecificity(selector),
              style: this.genStyleFromRule(rule)
            }]);
  ...
  StyleParser.prototype.genStyleTree = function (root) {
    ...
    while (stack.length) {
      ...
      for (var _i = 0, _b = node.children; _i < _b.length; _i++) {
        ...
        stack.push([child, __spreadArray(__spreadArray([], path), [node]), styleNode, rootStyleNode_1]);
      }
    }
  ...
  StyleParser.prototype.genKeySelector = function (ruleSel) {
    var selector = selectorToArr(ruleSel, /\s/).pop();
  ...
  StyleParser.prototype.computeCSS = function (dom, path, parentStyle) {
    ...
    var filteredRules = [];
    walkDomSelector(dom, function (keyName, selector) {
      switch (keyName) {
        case 'id':
    ...
    var finaleRules = filteredRules.filter(function (rule) {
      // 判断selector是否和路径匹配，
      var ruleSelectors = selectorToArr(rule.selector, /\s+/g); // 逆序判断每个子选择器
      ...
      for (; lastSelMatch > -1; lastSelMatch--) {
        ...
        for (; lastDomMatch > -1; lastDomMatch--) {
          ...
          if (isSelectorMatchDom(dom_1, ruleSelectors[lastSelMatch])) {
            isMatch = true;
            break;
          }
        }
    ...
    }, __assign({}, defaultStyle)); // 解析属性值/属性转驼峰
    ...
    for (var _i = 0, _a = Object.entries(finalStyle); _i < _a.length; _i++) {
      ...
      var parsedValue = parseAttr(key, value);

      if (_typeof(parsedValue) === 'object') {
        jsStyle = __assign(__assign({}, jsStyle), parsedValue);
      } else {
